...
const cylinderOptions = Array.from({ length: 41 }, (_, i) => {
  const value = ((i - 20) / 4).toFixed(2);
  const num = parseFloat(value);
  return {
    value,
...
const pdOptions = Array.from({ length: 71 }, (_, i) => {
  const value = (20 + i * 0.5).toFixed(1);
  return value;
});
...
const formData = ref({
  rightEye: {
    ...
    CYL: formatCylinderValue(props.ocrData.rightEye.CYL),
  ...
  leftEye: {
    ...
    CYL: formatCylinderValue(props.ocrData.leftEye.CYL),
...
function formatValue(value) {
  ...
  // Check if the value exists in sphereOptions, if not return "0.00"
  const existsInOptions = sphereOptions.some(option => option.value === formattedValue);
  return existsInOptions ? formattedValue : "0.00";
}

function formatCylinderValue(value) {
  if (!value || value === "") return "0.00";
  // Convert to number and format to 2 decimal places
  const num = parseFloat(value);
  if (isNaN(num)) return "0.00";
  ...
  const formattedValue = num.toFixed(2);
  // Check if the value exists in cylinderOptions, if not return "0.00"
  const existsInOptions = cylinderOptions.some(option => option.value === formattedValue);
  return existsInOptions ? formattedValue : "0.00";
}
...
function formatAxisValue(value) {
  ...
  // Check if the value exists in axisOptions, if not return "0"
  const existsInOptions = axisOptions.includes(formattedValue);
  return existsInOptions ? formattedValue : "0";
}
...
function formatAddValue(value) {
  ...
  const formattedValue = num.toFixed(2);
  // Check if the value exists in addOptions, if not return "0.00"
  const existsInOptions = addOptions.some(option => option.value === formattedValue);
  return existsInOptions ? formattedValue : "0.00";
}
...
function formatPdValue(value) {
  ...
  // Check if the value exists in pdOptions, if not return "0.0"
  const existsInOptions = pdOptions.includes(formattedValue);
  return existsInOptions ? formattedValue : "0.0";
}
    ...
    if (
      ...
      formData.value = {
        rightEye: {
          ...
          CYL: formatCylinderValue(newOcrData.rightEye.CYL),
        ...
        leftEye: {
          ...
          CYL: formatCylinderValue(newOcrData.leftEye.CYL),
